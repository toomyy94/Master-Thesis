...
public class ConnectionClassManager {
	...
	private final static Logger logger = LoggerFactory.getLogger(ConnectionClassManager.class);
  ...
  private double mCurrentBandwidthConnection = 0;
  ...
  static final long BANDWIDTH_LOWER_UPLOAD_BOUND = 1;
  
  private TrafficType trafficType = TrafficType.Download;
  
  private double startSampleTime = -1;
  private double accessTime = -1;
  
  
  // Singleton.
  private static class ConnectionClassManagerHolder {
      public static final ConnectionClassManager instance = new ConnectionClassManager(TrafficType.Download);
  }
   ...
   */
  public static ConnectionClassManager getInstance(TrafficType trafficType) {
      //return ConnectionClassManagerHolder.instance;
	  return new ConnectionClassManager(trafficType);
  }

  // Force constructor to be private.
  private ConnectionClassManager(TrafficType trafficType) {
  ...
  public synchronized void addBandwidth(long bytes, long timeInMs) {
	  final String method = "addBandwidth";
	  
    //Ignore garbage values.
    if (timeInMs == 0 || (bytes) * 1.0 / (timeInMs) * BYTES_TO_BITS < BANDWIDTH_LOWER_BOUND) {
    	MyLogger.trace(logger, method, "Ignore garbage values");
      return;
    }

    if (accessTime == -1)
    	accessTime = System.currentTimeMillis() - startSampleTime;
    ...
    if (mInitiateStateChange) {
      ...
      if (mSampleCounter >= DEFAULT_SAMPLES_TO_QUALITY_CHANGE  && significantlyOutsideCurrentBand()) {
        ...
        mCurrentBandwidthConnection = getCurrentBandwidth();
        notifyListeners();
      }
      return;
    }
  ...
  public synchronized double getCurrentBandwidth() {
  ...
  public interface ConnectionClassStateChangeListener {
    ...
    public void onBandwidthStateChange(double bandwidthState);
  }
  ...
  private void notifyListeners() {
    ...
    for (int i = 0; i < size; i++) {
      ...
      mListenerList.get(i).onBandwidthStateChange(mCurrentBandwidthConnection);
    }
  }
}
