...
#define APPNAME "ArQos"

jint
Java_pt_ptinovacao_arqospocket_service_audio_NativeTTAEnc_encodeWav(
        JNIEnv *env,        /* interface pointer */
        jobject obj,        /* "this" pointer */
        jstring path)       /* path to wav file to encode */
{
    /* Obtain a C-copy of the Java string */
    const char *str = (*env)->GetStringUTFChars(env, path, 0);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Lets encode");
    encodeFiles(str);

    //return (*env)->ReleaseStringUTFChars(env, COPYRIGHT, str);
//TODO return errors...
...
void tta_error(int error, wchar_t *name) {
    ...
    switch (error) {
        case COMMAND_ERROR:
            ...
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tunknown command '%s'\n%hs\n", "name", LINE);
            break;
        case FORMAT_ERROR:
            //fwprintf(stderr, L"Error:\tnot compatible file format\n%hs\n", LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tnot compatible file format\n%hs\n", LINE);
            break;
        case FILE_ERROR:
            //fwprintf(stderr, L"Error:\tfile is corrupted\n%hs\n", LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tfile is corrupted\n%hs\n", LINE);
            break;
        case FIND_ERROR:
            //fwprintf(stderr, L"Error:\tfile(s) not found '%ls'\n%hs\n\n", name, LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tfile(s) not found '%s'\n%hs\n\n", "name", LINE);
            //exit(1);
        case CREATE_ERROR:
            //fwprintf(stderr, L"Error:\tproblem creating directory '%ls'\n%hs\n\n", name, LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,   "Error:\tproblem creating directory '%s'\n%hs\n\n", "name", LINE);
            //exit(1);
        case OPENR_ERROR:
           // fwprintf(stderr, L"Error:\tcan't open input file '%ls'\n%hs\n\n", name, LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tcan't open input file '%s'\n%hs\n\n", "name", LINE);
            //exit(1);
        case OPENW_ERROR:
            //fwprintf(stderr, L"Error:\tcan't open output file '%ls'\n%hs\n\n", name, LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tcan't open output file '%s'\n%hs\n\n", "name", LINE);
            //exit(1);
        case MEMORY_ERROR:
            //fwprintf(stderr, L"Error:\tinsufficient memory available\n%hs\n\n", LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tinsufficient memory available\n%hs\n\n", LINE);
            //exit(1);
        case WRITE_ERROR:
            //fwprintf(stderr, L"Error:\tcan't write to output file\n%hs\n\n", LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tcan't write to output file\n%hs\n\n", LINE);
            //exit(1);
        case READ_ERROR:
            //fwprintf(stderr, L"Error:\tcan't read from input file\n%hs\n\n", LINE);
            __android_log_print(ANDROID_LOG_ERROR, APPNAME,  "Error:\tcan't read from input file\n%hs\n\n", LINE);
            //exit(1);
    }
}
...
void get_binary(unsigned int *value, unsigned int bits) {
...
void get_unary(unsigned int *value) {
    *value = 0;
...
void put_binary(unsigned int value, unsigned int bits) {
    while (bit_count >= 8) {
        if (bitpos == BIT_BUFFER_END) {
...
void put_unary(unsigned int value) {
    do {
        while (bit_count >= 8) {
            if (bitpos == BIT_BUFFER_END) {
                int res = fwrite(BIT_BUFFER, 1,
...
int done_buffer_write() {
    ...
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"1. frame_crc32 %d\n", frame_crc32);
    frame_crc32 = ENDSWAP_INT32(frame_crc32);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"2. frame_crc32 %d\n", frame_crc32);
...
void memshl(register int *pA, register int *pB) {
    *pA++ = *pB++;
    *pA++ = *pB++;
    *pA++ = *pB++;
    *pA++ = *pB++;
...
void hybrid_filter(fltst *fs, int *in, int mode) {
...
int compress(FILE *fdin, FILE *fdout) {
    ...
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.ChunkID %d\n", wave_hdr.ChunkID);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.ChunkSize %d\n", wave_hdr.ChunkSize);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.Subchunk1ID %d\n", wave_hdr.Subchunk1ID);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.Subchunk1Size %d\n", wave_hdr.Subchunk1Size);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.AudioFormat %d\n", wave_hdr.AudioFormat);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.NumChannels %d\n", wave_hdr.NumChannels);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.SampleRate %d\n", wave_hdr.SampleRate);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.ByteRate %d\n", wave_hdr.ByteRate);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.BlockAlign %d\n", wave_hdr.BlockAlign);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"VEFORE wave_hdr.BitsPerSample %d\n", wave_hdr.BitsPerSample);
    ...
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.ChunkID %d\n", wave_hdr.ChunkID);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.ChunkSize %d\n", wave_hdr.ChunkSize);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.Subchunk1ID %d\n", wave_hdr.Subchunk1ID);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.Subchunk1Size %d\n", wave_hdr.Subchunk1Size);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.AudioFormat %d\n", wave_hdr.AudioFormat);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.NumChannels %d\n", wave_hdr.NumChannels);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.SampleRate %d\n", wave_hdr.SampleRate);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.ByteRate %d\n", wave_hdr.ByteRate);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.BlockAlign %d\n", wave_hdr.BlockAlign);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"wave_hdr.BitsPerSample %d\n", wave_hdr.BitsPerSample);
    ...
    if (wave_hdr.Subchunk1Size > def_subchunk_size) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Encode:\tskiped %d extra format bytes\n", extra_len);
    }
    ...
    while (1) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Encode:\tskiped unsupported '%hs' chunk\n", chunk_id);
    }
    ...
    tta_hdr.DataLength = data_len;*/
    tta_hdr.CRC32 = crc32((unsigned char *) &tta_hdr,
                          sizeof(tta_hdr) - sizeof(int));
    tta_hdr.CRC32 = ENDSWAP_INT32(tta_hdr.CRC32);

    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"tta_hdr.TTAid %d\n", tta_hdr.TTAid);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"tta_hdr.AudioFormat %d\n", tta_hdr.AudioFormat);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"tta_hdr.NumChannels %d\n", tta_hdr.NumChannels);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"tta_hdr.BitsPerSample %d\n", tta_hdr.BitsPerSample);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"tta_hdr.SampleRate %d\n", tta_hdr.SampleRate);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"tta_hdr.DataLength %d\n", tta_hdr.DataLength);
    __android_log_print(ANDROID_LOG_ERROR, APPNAME,"tta_hdr.CRC32 %d\n", tta_hdr.CRC32);
    ...
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "Encode:\tcomplete, wrote %d bytes, ratio: %.2f, time: %d\n",
             (int) output_byte_count,
             (float) output_byte_count / (input_byte_count + 1),
             (int) (time(NULL) - stime));
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "%hs\n", LINE);
...
// TODO implement decompress to use for playback
...
void fill_out_name_with_extension(wchar_t *ext) {
    int len;
    wchar_t *p, *filename;
...
wchar_t * remove_filename_from_path(wchar_t *path) {
    wchar_t *p, *dirname;
    dirname = path;
    p = wcsrchr(dirname, _SEP);
    if (p) *p = L'\0';
...
void process_files(int act, int files) {
    ...
    for (item = files_list; item != NULL; item = item->next) {
       ...
       /* if (!fixed_out)
            switch (act) {
                case 1:
                    fill_out_name_with_extention(L".tta");
                    break;
                case 2:
                    fill_out_name_with_extension(L".wav");
                    break;
            }
*/
        // print file banner
        //fwprintf(stderr, L"File:\t[%ls]\n", print_path(file_in, 0));
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "File:\t[%s]\n", print_path(file_in, 0));
        ...
        switch (act) {
            case 1:
                ret = compress(fdin, fdout);
                break;
            case 2:
                ret = decompress(fdin, fdout);
                break;
        }
...
int encodeFiles(wchar_t *path_in){
    int act = 1, farg = 1;
    ...
    *out_path = *file_in = *file_out = L'\0';
    ...
    wstrncpy(file_in, path_in, _MAX_FNAME - 1);

    add_to_files_list(file_in, &files_list, &files_list_tail);
    if (!fixed_out)
        switch (act) {
            case 1:
                fill_out_name_with_extension(L".tta");
                break;
            case 2:
                fill_out_name_with_extension(L".wav");
                break;
