...
import pt.ptinovacao.arqospocket.interfaces.INextPage;
...
public class SwipeListViewTouchListener implements View.OnTouchListener {
     ...
     * @param position  Position of list
     */
    protected void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
     ...
     * @param position  Position of list
     */
    protected void generateDismissAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        int moveTo = 0;
        if (opened.get(position)) {
            if (!swap) {
     ...
     * @param position  list position
     */
    protected void generateRevealAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        int moveTo = 0;
        if (opened.get(position)) {
            if (!swap) {
                moveTo = openedRight.get(position) ? (int) (viewWidth - rightOffset) : (int) (-viewWidth + leftOffset);
...
//                Arrastar
                if(swap && !swapRight) {
//                	((INextPage) swipeListView.getContext()).nextPage(0);
                	((IFragmentChange) swipeListView.getContext()).changeFragment(downPosition);
                	Log.d("downPosition", downPosition + "");
                }
