...
public class ScanlogsAttachmentsProcessManager {

    private static final Logger LOGGER = LoggerFactory.getLogger(ScanlogsAttachmentsProcessManager.class);

    private static ScanlogsAttachmentsProcessManager attachmentsProcessManager;
    ...
    public static ScanlogsAttachmentsProcessManager getInstance(CoreApplication coreApplication) {

        if (attachmentsProcessManager == null) {
            attachmentsProcessManager = new ScanlogsAttachmentsProcessManager(coreApplication);
        }
        return attachmentsProcessManager;
    }

    public ScanlogsAttachmentsProcessManager(CoreApplication coreApplication) {
        this.coreApplication = coreApplication;
    }

    public static void startSendAttachment(CoreApplication coreApplication) {
        ScanlogsAttachmentsProcessManager instance = ScanlogsAttachmentsProcessManager.getInstance(coreApplication);

        if (instance.isRunning()) {
            LOGGER.debug("ScanlogAttachments In execution");
            return;
        }
                ...
                .subscribe(new Consumer<Boolean>() {
                    ...
                    public void accept(@NonNull Boolean isSuccess) throws Exception {
                        if (isSuccess) {
                            ...
                            executingEventDao.updateAllScanlogsThatUploadFile(taskEvent.getId()).blockingGet();
    ...
    public void notificationServer(final ArrayList<String> fileNames) {
        ...
        Single.just(client).subscribeOn(Schedulers.newThread()).subscribe(new Consumer<HttpClient>() {
            ...
            public void accept(@NonNull HttpClient httpClient) throws Exception {
                if (client.postProbeNotificationScanlogsAttachmentsProcess(fileNames).isSuccess()) {
                    ExecutingEventDao executingEventDao = coreApplication.getDatabaseHelper().createExecutingEventDao();
                    final List<Radiolog> taskEvents = executingEventDao.readAllScanlogsToSendNotification();

                    executingEventDao.updateAllScanlogsThatPostFileNotification(getIds(taskEvents)).blockingGet();
    ...
    public void start() {
        ...
        if (mode) {
            ...
            if (taskEvents.size() > 0) {
                ...
                if (checkPending()) {
                    mode = (executingEventDao.readAllScanlogsToSendNotification().size() > 0);
                    start();
                } else {
                    setRunning(false);
                }
            ...
            List<Radiolog> taskEvents = executingEventDao.readAllScanlogsToSendAttachments();
            if (taskEvents.size() > 0) {
    ...
    private boolean checkPending() {
        ...
        if (executingEventDao.readAllScanlogsToSendAttachments().size() > 0 ||
                executingEventDao.readAllScanlogsToSendNotification().size() > 0) {
            return true;
        }
        return false;
    }
    ...
    public static class AttachmentReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            ScanlogsAttachmentsProcessManager.getInstance(applicationContext).start();
        }
    }
