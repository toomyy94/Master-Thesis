...
import pt.ptinovacao.arqospocket.core.tests.tasks.SendMailExecutableTask;

/**
 * Send Mail Task Resolver.
 * <p>
 * Created by Emílio Simões on 17-04-2017.
 */
public class SendMailTaskResolver implements InstanceResolver {

    private static final ResolverInfo RESOLVER_INFO = new ResolverInfo(TaskType.SEND_MAIL_TASK);

    @Override
    public TaskData task() {
        return new SendMailTaskData(RESOLVER_INFO);
    }

    @Override
    public TaskResult result() {
        return new SendMailTaskResult(RESOLVER_INFO);
    }

    @Override
    public BaseExecutableTask executableTask(TaskData data) {
        return new SendMailExecutableTask(data);
    }
