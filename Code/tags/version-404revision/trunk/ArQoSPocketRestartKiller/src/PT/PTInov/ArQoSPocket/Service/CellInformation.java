...
import PT.PTInov.ArQoSPocket.Utils.ObservedCellObject;
...
public class CellInformation implements WacthDogInterface {
	...
	private List<ObservedCellObject> observedCells = null;
	...
	public CellInformation(Context c) {
		try {
			...
			if (telephonyManager != null) {
				...
				//setObservedCells(telephonyManager.getAllCellInfo());
			}
	...
	private void setObservedCells(List<CellInfo> cellInfo) {
		final String method = "setObservedCells";
		
		Logger.v(tag, method, LogType.Trace, "In");

		try {
			
			ArrayList<ObservedCellObject> localObservedCells = new ArrayList<ObservedCellObject>();
			
			for(CellInfo c :cellInfo) {
				CellInfoGsm localc = (CellInfoGsm)c;
				localObservedCells.add(new ObservedCellObject(localc.getCellSignalStrength().getAsuLevel(),localc.getCellSignalStrength().getDbm(),
						localc.getCellIdentity().getCid(), localc.getCellIdentity().getLac(), localc.getCellIdentity().getMcc(), localc.getCellIdentity().getMnc(), 
						localc.getCellIdentity().getPsc()) );
			}
			
			observedCells = localObservedCells;
	...
	public List<ObservedCellObject> getObservedCells() {
		...
		public void onSignalStrengthsChanged(SignalStrength signalStrength) {
			...
			try {
				...
				if (signalStrength.isGsm()) {
					gsmSignalStrength = convertToDBM(signalStrength
