...
public class SwipeListViewTouchListenerTeste implements View.OnTouchListener {

	protected static final int DISPLACE_CHOICE = 80;

	protected int swipeMode = SwipeListView.SWIPE_MODE_BOTH;
	protected boolean swipeOpenOnLongPress = true;
	protected boolean swipeClosesAllItemsWhenListMoves = true;
	...
	protected long configShortAnimationTime;
	protected long animationTime;

	protected float leftOffset = 0;
	protected float rightOffset = 0;

	protected int swipeDrawableChecked = 0;
	protected int swipeDrawableUnchecked = 0;

	// Fixed properties
	protected SwipeListViewTeste swipeListView;
	protected int viewWidth = 1; // 1 and not 0 to prevent dividing by zero

	protected List<PendingDismissData> pendingDismisses = new ArrayList<PendingDismissData>();
	protected int dismissAnimationRefCount = 0;

	protected float downX;
	protected boolean swiping;
	protected boolean swipingRight;
	...
	protected int swipeCurrentAction = SwipeListView.SWIPE_ACTION_NONE;

	protected int swipeActionLeft = SwipeListView.SWIPE_ACTION_REVEAL;
	protected int swipeActionRight = SwipeListView.SWIPE_ACTION_REVEAL;

	protected List<Boolean> opened = new ArrayList<Boolean>();
	protected List<Boolean> openedRight = new ArrayList<Boolean>();
	protected boolean listViewMoving;
	protected List<Boolean> checked = new ArrayList<Boolean>();
	protected int oldSwipeActionRight;
	protected int oldSwipeActionLeft;
	ImageView b, b1, b2, b3 ;
	
	/**
	 * Constructor
	 * 
	 * @param swipeListViewTeste
	 *            SwipeListView
	 ...
	 */
	public SwipeListViewTouchListenerTeste(
			SwipeListViewTeste swipeListViewTeste, int swipeFrontView,
			int swipeBackView) {
		
		this.swipeFrontView = swipeFrontView;
		this.swipeBackView = swipeBackView;
		ViewConfiguration vc = ViewConfiguration.get(swipeListViewTeste
				.getContext());
		slop = vc.getScaledTouchSlop();
		minFlingVelocity = vc.getScaledMinimumFlingVelocity();
		maxFlingVelocity = vc.getScaledMaximumFlingVelocity();
		configShortAnimationTime = swipeListViewTeste.getContext()
				.getResources()
				.getInteger(android.R.integer.config_shortAnimTime);
		animationTime = configShortAnimationTime;
		this.swipeListView = swipeListViewTeste;
	}
	...
	protected void setFrontView(View frontView) {
		...
		b = (ImageView) frontView.findViewById(R.id.imagebutton);
//		LinearLayout L= (LinearLayout) frontView.findViewById(R.id.linearSlide);
//		L.setPadding(0,0,-300,0);
...
//		L.setVisibility(View.VISIBLE);
	 ...
	 *            position of list
	 */
	protected void swapChoiceState(int position) {
	 ...
	 *            Position of list
	 */
	protected void openAnimate(View view, int position) {
	 ...
	 *            Position of list
	 */
	protected void closeAnimate(View view, int position) {
		if (opened.get(position)) {
			generateRevealAnimate(view, true, false, position);
		}
	}
	 ...
	 *            list position
	 */
	protected void generateChoiceAnimate(final View view, final int position) {
	 ...
	 *            Position of list
	 */
	protected void generateDismissAnimate(final View view, final boolean swap,
			final boolean swapRight, final int position) {
		int moveTo = 0;
		if (opened.get(position)) {
			if (!swap) {
	 ...
	 *            list position
	 */
	protected void generateRevealAnimate(final View view, final boolean swap,
			final boolean swapRight, final int position) {
		int moveTo = 0;
		if (opened.get(position)) {
			if (!swap) {
			...
			if (deltaMode > slop
				...
				if (opened.get(downPosition)) {
					...
					if (swipingRight
						...
						// backView.findViewById(R.id.imagebutton2);
						// ImageView b3 = (ImageView)
						// backView.findViewById(R.id.imagebutton3);
						// b.setVisibility(View.VISIBLE);
...
//				ImageView txt =(ImageView) frontView.findViewById(R.id.imagebutton3);
